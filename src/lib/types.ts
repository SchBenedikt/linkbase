...
export type Link = {
  ...
  orderIndex: number;
  title: string;
  type: 'link' | 'text' | 'spotify' | 'youtube' | 'article' | 'blog-overview' | 'image' | 'product' | 'profile' | 'map' | 'instagram' | 'tiktok' | 'soundcloud' | 'vimeo' | 'clock' | 'countdown' | 'calendly' | 'github' | 'twitter' | 'twitch' | 'donation' | 'contact-info' | 'audio' | 'appdownload' | 'pinterest' | 'discord' | 'testimonial' | 'faq' | 'header' | 'video' | 'qrcode' | 'linkedin';
  colSpan?: number;
  rowSpan?: number;
