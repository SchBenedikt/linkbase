...
export function InstagramCard({ link, onEdit, onDelete, isEditable = false, appearance, dragHandleListeners }: InstagramCardProps) {
  const getInstagramPostId = (url: string): string | null => {
    const match = url.match(/instagram\.com\/p\/([A-Za-z0-9_-]+)/);
    return match ? match[1] : null;
  };

  const postId = link.url ? getInstagramPostId(link.url) : null;
  ...
  return (
    ...
    >
      <iframe
        title={`Instagram Embed: ${link.title}`}
        src={embedUrl}
        width="100%"
        height="100%"
        scrolling="no"
