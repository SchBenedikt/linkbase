...
export function CalendlyCard({ link, onEdit, onDelete, isEditable = false, appearance, dragHandleListeners }: CalendlyCardProps) {
  ...
  if (!link.url) {
    return (
      <Card
        ...
        style={cardStyle}
      >
        <p className="text-center text-card-foreground/50">Calendly URL is missing.</p>
      </Card>
    );
  }
  ...
  return (
    ...
    >
      ...
      <div className="relative flex-grow">
        <iframe
          title={link.title || 'Calendly Booking'}
          src={link.url}
          width="100%"
          height="100%"
          loading="lazy"
