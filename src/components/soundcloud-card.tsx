...
export function SoundcloudCard({ link, onEdit, onDelete, isEditable = false, appearance, dragHandleListeners }: SoundcloudCardProps) {
  ...
  if (!link.url) {
    return (
      <Card
        ...
        style={cardStyle}
      >
        <p className="text-center text-card-foreground/50">SoundCloud URL is missing.</p>
      </Card>
    );
  }

  const embedUrl = `https://w.soundcloud.com/player/?url=${encodeURIComponent(link.url)}&color=%23ff5500&auto_play=false&hide_related=true&show_comments=false&show_user=true&show_reposts=false&show_teaser=false`;
  ...
  return (
    ...
    >
      <iframe
        title={`SoundCloud Embed: ${link.title}`}
        src={embedUrl}
        width="100%"
        height="100%"
        allow="autoplay"
